...
def analyze_journal(file_path):
    ...
    if len(dates) != len(posts):
        print(
            f"Warning: Mismatch in {file_path.name}: {len(dates)} dates vs {len(posts)} posts"
        )
    ...
    for date, post_list in date_posts.items():
        ...
        entries.append({"date": date, "words": word_count})
    ...
    if total_entries > 1:
        ...
        if current_sequence_length >= 2:
            ...
            longest_streak = (
                max(longest_streak, 1) if longest_streak == 0 else longest_streak
            )
    ...
    continuity_percentage = (
        int((streak_days_total / total_entries) * 100) if total_entries > 0 else 0
    )
...
def main():
    ...
    print(f"Analyzing {len(xml_files)} blogs...")
